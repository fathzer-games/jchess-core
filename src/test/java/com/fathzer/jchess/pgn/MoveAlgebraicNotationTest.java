...
class MoveAlgebraicNotationTest {
	...
	void test() {
		...
		assertEquals("exd4", san.get(board, get(board, "e5","d4")));
		...
		assertEquals("Qxd2+", san.get(board, get(board, "b4","d2")));
		...
		assertEquals("Qa3#", san.get(board, get(board, "c1","a3")));
		
		board = FENUtils.from("r3k2r/pppnqppp/3b1n2/5b2/8/4P3/PPP2PPP/RNBQKBNR b KQkq - 3 6");
		assertEquals("O-O", san.get(board, get(board, "e8","g8")));
		board = FENUtils.from("r3k2r/pppnqppp/3b1n2/5b2/8/4P3/PPP2PPP/RNBQKBNR b KQkq - 3 6");
		assertEquals("O-O-O", san.get(board, get(board, "e8","c8")));
		
		board = FENUtils.from("rnbqkbnr/p1pppppp/8/PpP5/8/8/1P1PPPPP/RNBQKBNR w KQkq b6 0 1");
		assertEquals("axb6 e.p.", san.get(board, get(board, "a5","b6")));
		assertNull(board.getPiece(cs.getIndex("b5")));
		
		board = FENUtils.from("2kr3r/Ppp1pppp/3p4/8/2P5/1P3K2/2PP2PP/R1B1Q3 w - - 0 1");
		assertEquals("a8=Q+",san.get(board, get(board, "a7","a8", Piece.WHITE_QUEEN)));
		
		board = FENUtils.from("4k3/8/8/8/8/8/r5q1/4K3 b - - 0 1");
		assertEquals("Rd2",san.get(board, get(board, "a2","d2")),"Problem with DRAW");
		
		// Disambiguation
		board = FENUtils.from("2kr3r/pppppppp/8/R7/2P1Q2Q/1P3K2/2PP2PP/RNB4Q w - - 0 1");
		san.withPlayMove(false);
		assertEquals("R1a3", san.get(board, get(board, "a1","a3")));
		san.withPlayMove(true);
		assertEquals("Qh4e1", san.get(board, get(board, "h4","e1")));
		assertEquals("Rdf8", san.get(board, get(board, "d8","f8")));
		
		// Illegal moves
		final Board<Move> board2 = board.create();
		board2.copy(board);
		final Move move2 = get(board, "f3","g2");
		...
		final Move move3 = get(board, "a7","a8", Piece.WHITE_QUEEN);
	...
	void testCustom() {
		...
		Board<Move> board = FENUtils.from("rnbqkbnr/pppp1ppp/8/4p3/3P3P/8/PPP1PPP1/RNBQKBNR b KQkq d3 0 2");
		final CoordinatesSystem cs = board.getCoordinatesSystem();
		assertEquals("e:d4", san.get(board, get(board, "e5","d4")));
		assertEquals(Piece.WHITE_PAWN, board.getPiece(cs.getIndex("d4")));
		
		board = FENUtils.from("r1b1k2r/ppp2ppp/5n2/7P/Pq1n4/6P1/1P1Q1P2/1R2KBNR b Kkq - 1 13");
		san.withPlayMove(true);
		assertEquals("Q:d2ch", san.get(board, get(board, "b4","d2")));
		assertEquals(Piece.BLACK_QUEEN, board.getPiece(cs.getIndex("d2")));
		...
		board = FENUtils.from("r3k2r/pppnqppp/3b1n2/5b2/8/4P3/PPP2PPP/RNBQKBNR b KQkq - 3 6");
		assertEquals("0-0", san.get(board, get(board, "e8","g8")));
		board = FENUtils.from("r3k2r/pppnqppp/3b1n2/5b2/8/4P3/PPP2PPP/RNBQKBNR b KQkq - 3 6");
		assertEquals("0-0-0", san.get(board, get(board, "e8","c8")));
		
		board = FENUtils.from("rnbqkbnr/p1pppppp/8/PpP5/8/8/1P1PPPPP/RNBQKBNR w KQkq b6 0 1");
		assertEquals("a:b6", san.get(board, get(board, "a5","b6")));
		assertNull(board.getPiece(cs.getIndex("b5")));
		
		board = FENUtils.from("2kr3r/Ppp1pppp/3p4/8/2P5/1P3K2/2PP2PP/R1B1Q3 w - - 0 1");
		assertEquals("a8Qch",san.get(board, get(board, "a7","a8", Piece.WHITE_QUEEN)));
	}
}
