...
class MinimaxEngineTest {
	...
	void blackPlayingTest() {
		...
		Board<Move> board = FENParser.from("7k/5p1Q/5P1N/5PPK/6PP/8/8/8 b - - 6 5");
		List<Evaluation<Move>> moves = mme4.getBestMoves(board, Integer.MAX_VALUE, 0);
System.out.println(moves);
		assertEquals(1, moves.size());
		assertEquals("h8", Coord.toString(moves.get(0).getContent().getFrom()));
		assertEquals("h7", Coord.toString(moves.get(0).getContent().getTo()));
...
System.out.println(moves);
		...
		assertEquals("c3", Coord.toString(mv.getFrom()));
		assertEquals("c2", Coord.toString(mv.getTo()));
...
System.out.println(moves);
		...
		assertEquals("b3", Coord.toString(mv.getFrom()));
		assertEquals("a1", Coord.toString(mv.getTo()));
...
System.out.println(moves);
		assertEquals(getMateScore(2), moves.get(0).getValue());
		assertTrue(moves.get(1).getValue()<moves.get(0).getValue());
		assertEquals("g6", Coord.toString(moves.get(0).getContent().getFrom()));
		assertEquals("h8", Coord.toString(moves.get(0).getContent().getTo()));
...
assertEquals(19, moves.size());
		mv = moves.get(0).getContent();
		assertEquals("d1", Coord.toString(mv.getFrom()));
		assertEquals("d7", Coord.toString(mv.getTo()));
	}
	
	@Test
	void moreTests() {
		Board<Move> board = FENParser.from("8/8/8/3kr3/8/8/5PPP/7K w - - 0 1");
		...
		l.add(Coord.toIndex("h1"), Coord.toIndex("g1"));
		l.add(Coord.toIndex("f2"), Coord.toIndex("f3"));
		l.add(Coord.toIndex("f2"), Coord.toIndex("f4"));
