...
class StartPositionGeneratorTest {
	...
	void test() {
		...
		String fen518 = FENUtils.to(StartPositionGenerator.INSTANCE.fromPositionNumber(518));
		assertEquals(FENUtils.NEW_STANDARD_GAME, fen518);
		
		// Test there is 960 different positions
		final Set<String> fens = IntStream.range(0, 960).mapToObj(i -> FENUtils.to(StartPositionGenerator.INSTANCE.fromPositionNumber(i))).collect(Collectors.toSet());
