...
class ChessBoardTest {
	...
	void castlingsTest() {
		...
		copy.copy(board);
		copy.move(new SimpleMove(cs, "e8","d8"));
		assertFalse(copy.hasCastling(Castling.BLACK_QUEEN_SIDE));
		assertFalse(copy.hasCastling(Castling.BLACK_KING_SIDE));
		assertEquals(Piece.BLACK_KING,copy.getPiece(cs.getIndex("d8")));
		assertNull(copy.getPiece(cs.getIndex("e8")));

		// Test castling moves the pieces has it should
		copy.copy(board);
		copy.move(new SimpleMove(cs, "e1","g1"));
		assertEquals(Piece.WHITE_KING,copy.getPiece(cs.getIndex("g1")));
		assertEquals(cs.getIndex("g1"), copy.getKingPosition(Color.WHITE));
		assertEquals(Piece.WHITE_ROOK,copy.getPiece(cs.getIndex("f1")));
		assertNull(copy.getPiece(cs.getIndex("h1")));
		assertNull(copy.getPiece(cs.getIndex("e1")));
		assertFalse(copy.hasCastling(Castling.WHITE_KING_SIDE));
		assertFalse(copy.hasCastling(Castling.WHITE_QUEEN_SIDE));

		copy.copy(board);
		copy.move(new SimpleMove(cs, "e1","c1"));
		assertEquals(Piece.WHITE_KING,copy.getPiece(cs.getIndex("c1")));
		assertEquals(cs.getIndex("c1"), copy.getKingPosition(Color.WHITE));
		assertEquals(Piece.WHITE_ROOK,copy.getPiece(cs.getIndex("d1")));
		assertNull(copy.getPiece(cs.getIndex("a1")));
		assertNull(copy.getPiece(cs.getIndex("e1")));
		assertFalse(copy.hasCastling(Castling.WHITE_KING_SIDE));
		assertFalse(copy.hasCastling(Castling.WHITE_QUEEN_SIDE));
		...
		copy.copy(board);
		copy.move(new SimpleMove(cs, "a1","b1"));
		assertFalse(copy.hasCastling(Castling.WHITE_QUEEN_SIDE));
		assertTrue(copy.hasCastling(Castling.WHITE_KING_SIDE));
		copy.copy(board);
		copy.move(new SimpleMove(cs, "h1","h2"));
		assertFalse(copy.hasCastling(Castling.WHITE_KING_SIDE));
		assertTrue(copy.hasCastling(Castling.WHITE_QUEEN_SIDE));
		copy.move(new SimpleMove(cs, "a8","a7"));
		assertFalse(copy.hasCastling(Castling.BLACK_QUEEN_SIDE));
		assertTrue(copy.hasCastling(Castling.BLACK_KING_SIDE));
		assertTrue(copy.hasCastling(Castling.WHITE_QUEEN_SIDE));
		copy.move(new SimpleMove(cs, "h8","g8"));
		assertFalse(copy.hasCastling(Castling.BLACK_KING_SIDE));
	}
	...
	void pawnMoveTest() {
		...
		assertNull(copy.getPiece(cs.getIndex("b5")));
		assertNull(copy.getPiece(cs.getIndex("a5")));
		assertTrue(copy.getEnPassant()<0);
		assertEquals(0, copy.getHalfMoveCount());
		// Test promotion
		assertEquals(Color.BLACK, copy.getActiveColor());
		copy.move(new SimpleMove(cs, "g2","h1",Piece.BLACK_QUEEN));
		assertEquals(Piece.BLACK_QUEEN, copy.getPiece(cs.getIndex("h1")));
		assertNull(copy.getPiece(cs.getIndex("g2")));
		assertEquals(Color.WHITE, copy.getActiveColor());
		
		board = FENParser.from("4k1r1/1P6/5p2/p1Np1P2/5B1p/5Q1P/1q3PPK/8 w - - 4 42");
		board.move(new SimpleMove(cs, "b7","b8", Piece.WHITE_QUEEN));
		...
		board.move(new SimpleMove(cs, "c7","c5"));
	...
	void testRookMoveAndCastlings() {
		...
		final CoordinatesSystem cs = board.getCoordinatesSystem();
		board.move(new SimpleMove(cs, "a8","d8"));
		assertFalse(board.hasCastling(Castling.BLACK_QUEEN_SIDE));

		board = (ChessBoard) FENParser.from("4k2r/6K1/8/8/8/8/8/8 b k - 0 1");
		board.move(new SimpleMove(cs, "h8","f8"));
		assertFalse(board.hasCastling(Castling.BLACK_KING_SIDE));
		
		board = (ChessBoard) FENParser.from("4k2r/6b1/8/8/8/8/8/R3K2R w KQk - 0 1");
		board.move(new SimpleMove(cs, "h1", "g2"));
		assertFalse(board.hasCastling(Castling.WHITE_KING_SIDE));
		
		board = (ChessBoard) FENParser.from("4k2r/6b1/8/8/8/8/8/R3K2R w KQk - 0 1");
		board.move(new SimpleMove(cs, "a1","a8"));
		assertFalse(board.hasCastling(Castling.WHITE_QUEEN_SIDE));
	}
	...
	void testZobristKey() {
		...
		board.move(new SimpleMove(cs, "a3","c4"));
		assertEquals(FENParser.from("rnbqkbnr/pppppppp/8/1B6/2N5/3PPN2/PPPQBPPP/R3K2R b KQkq - 0 1").getKey(), board.getKey());
		board.move(new SimpleMove(cs, "g8","f6"));
		board.move(new SimpleMove(cs, "c4","a3"));
		board.move(new SimpleMove(cs, "f6","g8"));
		assertEquals(initial, board.getKey());
		
		// Test castling erase
		// 1 - Rook move
		board.move(new SimpleMove(cs, "h1","g1"));
		board.move(new SimpleMove(cs, "g8","f6"));
		board.move(new SimpleMove(cs, "g1","h1"));
		board.move(new SimpleMove(cs, "f6","g8"));
		...
		board.move(new SimpleMove(cs, "e1","d1"));
		board.move(new SimpleMove(cs, "g8","f6"));
		board.move(new SimpleMove(cs, "d1","e1"));
		board.move(new SimpleMove(cs, "f6","g8"));
		...
		// Test capture
		board.move(new SimpleMove(cs, "b5", "d7"));
		...
		board.move(new SimpleMove(cs, "g4", "h3"));
		...
		board.move(new SimpleMove(cs, "h7", "h8", Piece.WHITE_QUEEN));
