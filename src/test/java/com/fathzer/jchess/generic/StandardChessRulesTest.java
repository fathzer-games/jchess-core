...
class StandardChessRulesTest {
	...
	void pawnsTest() {
		...
		assertEquals(Set.of("a6","b6","c6"), getTo(cs, getMoves(cs, whiteMoves, "b5")));
	...
	private List<Move> getMoves(Board<Move> board, String from) {
		return getMoves(board.getCoordinatesSystem(), getMoves(board),from);
	}
...
//	@Test
	void speedTest() {
		...
		final CoordinatesSystem cs = wBoard.getCoordinatesSystem();
		...
		System.out.println(whiteMoves.stream().map(m -> toString(cs, m)).collect(Collectors.toList()));
	}
	
	private String toString(CoordinatesSystem cs, Move m) {
		return cs.getAlgebraicNotation(m.getFrom())+"-"+cs.getAlgebraicNotation(m.getTo());
	}
}
