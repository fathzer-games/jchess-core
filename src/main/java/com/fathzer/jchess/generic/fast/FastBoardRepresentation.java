...
public class FastBoardRepresentation {
	@Getter
	...
	public FastBoardRepresentation(Dimension dimension, List<PieceWithPosition> pieces) {
		...
		this.coordinatesSystem = new FastCoordinatesSystem(dimension);
		final int size = dimension.getHeight()*(dimension.getWidth()+2);
	...
	public void copy(FastBoardRepresentation other) {
	...
	BoardExplorer getExplorer(int pos) {
		return new FastBoardExplorer(pieces, coordinatesSystem, pos);
	}
}
