...
class AttackDetector {
	...
	private boolean check(DirectionExplorer explorer, Collection<Direction> directions, int maxIteration, Color color, Predicate<Piece> validator) {
		return directions.stream().anyMatch(d -> check(explorer, d, maxIteration, color, validator));
	}
	
	private boolean check(DirectionExplorer explorer, Direction direction, int maxIteration, Color color, Predicate<Piece> validator)  {
