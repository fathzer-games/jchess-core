...
class AttackDetector {
	...
	public boolean isAttacked(int position, Color color) {
		final InternalBoardExplorer explorer = new SkipFirstExplorer(board.getCoordinatesSystem(), position);
	...
	private boolean check(InternalBoardExplorer explorer, Collection<Direction> directions, int maxIteration, Color color, Predicate<Piece> validator) {
		return directions.stream().anyMatch(d -> check(explorer, d, maxIteration, color, validator));
	}
	
	private boolean check(InternalBoardExplorer explorer, Direction direction, int maxIteration, Color color, Predicate<Piece> validator)  {
